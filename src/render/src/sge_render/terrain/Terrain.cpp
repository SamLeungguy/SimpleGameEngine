...
private:
	void _init(int lod_, Vec2i vertexSize_)
	{
		...
		u32 halfWidth = vertexSize.x / 2;
		stride0 = (halfWidth + halfWidth - 1) * 3;
	...
	void _createTopBottonEdgeIndices()
	{
		...
		{
			auto start = i * 2;
			_emplace_back_TopBottom(lod0Indices, start + 0, 0, start + 2, 0, start + 1, 1);
			_emplace_back_TopBottom(lod0Indices, start + 2, 0, start + 3, 1, start + 1, 1);

			//_lod0_emplace_back_TopBottom(start,		0); _lod0_emplace_back_TopBottom(start + 2,	0); _lod0_emplace_back_TopBottom(start + 1, 1);
			//_lod0_emplace_back_TopBottom(start + 2,	0); _lod0_emplace_back_TopBottom(start + 3,	1); _lod0_emplace_back_TopBottom(start + 1, 1);

			if (i == end - 1)
			{
				_emplace_back_TopBottom(lod0Indices, start + 2, 0,start + 4, 0,start + 3, 1);
				//_lod0_emplace_back_TopBottom(start + 2, 0); _lod0_emplace_back_TopBottom(start + 4, 0); _lod0_emplace_back_TopBottom(start + 3, 1);
			}
		}
		...
		{
			auto start = i * 2;
			_emplace_back_TopBottom(lod1Indices, start + 0, 0, start + 1, 0, start + 1,	1);		// \|
			_emplace_back_TopBottom(lod1Indices, start + 1, 0, start + 2, 0, start + 1,	1);		//   |/
			_emplace_back_TopBottom(lod1Indices, start + 2, 0, start + 3, 1, start + 1,	1);		//    /\

			//_lod1_emplace_back_TopBottom(start + 0,	0); _lod1_emplace_back_TopBottom(start + 1,	0); _lod1_emplace_back_TopBottom(start + 1,	1);	// \|
			//_lod1_emplace_back_TopBottom(start + 1,	0); _lod1_emplace_back_TopBottom(start + 2,	0); _lod1_emplace_back_TopBottom(start + 1,	1);		//   |/
			//_lod1_emplace_back_TopBottom(start + 2,	0); _lod1_emplace_back_TopBottom(start + 3,	1); _lod1_emplace_back_TopBottom(start + 1,	1);		//    /\

			if (i == end - 1)
			{
				//_lod1_emplace_back_TopBottom(start + 2,	0); _lod1_emplace_back_TopBottom(start + 3,	0); _lod1_emplace_back_TopBottom(start + 3,	1);
				//_lod1_emplace_back_TopBottom(start + 3,	0); _lod1_emplace_back_TopBottom(start + 4,	0); _lod1_emplace_back_TopBottom(start + 3,	1);

				_emplace_back_TopBottom(lod1Indices, start + 2, 0, start + 3, 0, start + 3, 1);
				_emplace_back_TopBottom(lod1Indices, start + 3, 0, start + 4, 0, start + 3, 1);
			}
		}
	...
	void _createLeftEightEdgeIndices()
	{
		...
		for (int i = 0; i < end; i++)
		{
			...
			//_lod0_emplace_back_LeftRight(0,	start);		_lod0_emplace_back_LeftRight(1,	start + 1); _lod0_emplace_back_LeftRight(0,	start + 2);
			//_lod0_emplace_back_LeftRight(1,	start + 1);	_lod0_emplace_back_LeftRight(1,	start + 3); _lod0_emplace_back_LeftRight(0,	start + 2);

			if (i == end - 1)
			{
				_emplace_back_LeftRight(lod0Indices, 0, start + 2, 1, start + 3, 0, start + 4);
				//_lod0_emplace_back_LeftRight(0,	start + 2); _lod0_emplace_back_LeftRight(1,	start + 3); _lod0_emplace_back_LeftRight(0,	start + 4);
			}
		}
		_mergeAndClearIndicesChunk(lod0Indices, lod0Indices.size(), tmpIndices);
		...
		for (int i = 0; i < end; i++)
		{
			...
			//_lod1_emplace_back_LeftRight(0,		start); _lod1_emplace_back_LeftRight(1,	start + 1); _lod1_emplace_back_LeftRight(0,	start + 1);	// \|
			//_lod1_emplace_back_LeftRight(0,	start + 1); _lod1_emplace_back_LeftRight(1,	start + 1); _lod1_emplace_back_LeftRight(0,	start + 2);	//   |/
			//_lod1_emplace_back_LeftRight(1,	start + 1);	_lod1_emplace_back_LeftRight(1,	start + 3); _lod1_emplace_back_LeftRight(0,	start + 2);
			...
			if (i == end - 1)
			{
				...
				//_lod1_emplace_back_LeftRight(0,	start + 2); _lod1_emplace_back_LeftRight(1,	start + 3); _lod1_emplace_back_LeftRight(0,	start + 3);
				//_lod1_emplace_back_LeftRight(0,	start + 3); _lod1_emplace_back_LeftRight(1,	start + 3); _lod1_emplace_back_LeftRight(0,	start + 4);
			}
		}
		_mergeAndClearIndicesChunk(lod1Indices, lod1Indices.size(), tmpIndices);
	}
...
public:
	...
	void generate(IndexChunks& indexChunks_, Vec2i vertexSize_)
	{
		...
		lodIndices.createEdgeIndices(_currentLod, vertexSize_);

#if 1
		auto* renderer = Renderer::instance();
		
		int tblr = 0b00000000;
		auto& indicesChunks = (*pOut)[_currentLod];
...
void Terrain::_initMesh(CreateDesc& desc_, RenderMesh& outMesh_)
{
	...
	u8 tblr = 0b00000000;
	outMesh_.subMeshes()[0].setIndexBuffer(_indexChunks[lodIndex][tblr + 0], RenderDataTypeUtil::get<IndexType>());
